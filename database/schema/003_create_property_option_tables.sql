...
(
    ...
    `default_value`           VARCHAR(5)   NULL COMMENT '기본 옵션 값. true 또는 false. 미확인 시 NULL',
...
(
    ...
    `option_value`          VARCHAR(5)   NOT NULL COMMENT '옵션 값: true 또는 false',
...
(
    ...
    `change_type`                VARCHAR(30)  NOT NULL COMMENT 'CREATE, UPDATE, SOFT_DELETE',
    `changed_fields`             VARCHAR(500) NOT NULL COMMENT '변경 필드명 목록. 쉼표 구분 문자열',

    `before_value`               VARCHAR(5)   NULL COMMENT '변경 전 옵션 값: true 또는 false',
    `after_value`                VARCHAR(5)   NULL COMMENT '변경 후 옵션 값: true 또는 false',
